...
CZPltStake::CZPltStake(const libzerocoin::CoinSpend& spend)
{
...
int CZPltStake::GetChecksumHeightFromMint()
{
...
int CZPltStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZPltStake::GetChecksum()
{
    return nChecksum;
}

// The zPLT block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZPltStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZPltStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CZPltStake::GetUniqueness()
{
    //The unique identifier for a zPLT is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZPltStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZPltStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zPLT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPLTOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPLT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPLT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPLTOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPLT output", __func__);
...
bool CZPltStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZPltStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzPLTTracker* zpltTracker = pwallet->zpltTracker.get();
    CMintMeta meta;
    if (!zpltTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpltTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!PLT Stake
bool CPltStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CPltStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CPltStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CPltStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CPltStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CPltStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CPltStake::GetUniqueness()
{
    //The unique identifier for a PLT stake is the outpoint
...
CBlockIndex* CPltStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
