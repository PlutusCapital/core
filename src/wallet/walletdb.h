...
public:
    ...
    bool WriteZPLTSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZPLTSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZPLTSeed_deprecated(uint256& seed);
    bool EraseZPLTSeed();
    bool EraseZPLTSeed_deprecated();

    bool WriteZPLTCount(const uint32_t& nCount);
    bool ReadZPLTCount(uint32_t& nCount);
