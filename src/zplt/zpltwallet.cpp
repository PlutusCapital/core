// Copyright (c) 2017-2019 The PLUTUS developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zpltwallet.h"
#include "main.h"
#include "txdb.h"
...
CzPLTWallet::CzPLTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zplt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPLTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPLTSeed_deprecated()) {
                    LogPrintf("%s: Updated zPLT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zplt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zplt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPLTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPLTCount(nCountLastUsed);
    else if (!walletdb.ReadZPLTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPLTWallet::Lock()
{
    seedMaster = 0;
}

void CzPLTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPLTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPLT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPLTWallet::LoadMintPoolFromDB()
{
...
void CzPLTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPLTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPLTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpltTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPLTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPLT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpltTracker which also adds to database
    pwalletMain->zpltTracker->Add(dMint, true);
...
void CzPLTWallet::SeedToZPLT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPLTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPLTWallet::GenerateDeterministicZPLT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPLTWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPLTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPLTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
