// Copyright (c) 2018-2019 The PLUTUS developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zplt/deterministicmint.h>
#include "zplttracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzPLTTracker::CzPLTTracker(std::string strWalletFile)
{
...
CzPLTTracker::~CzPLTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPLTTracker::Init()
{
...
bool CzPLTTracker::Archive(CMintMeta& meta)
{
...
bool CzPLTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPLTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPLTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPLTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPLTTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPLTTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPLTTracker::GetSerialHashes()
{
...
CAmount CzPLTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPLTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPLTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPLTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPLTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPLTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPLTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPLTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPLTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPLTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPLTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPLTWallet* zPLTWallet)
{
    bool iszPLTWalletInitialized = (NULL != zPLTWallet);
    CMintMeta meta;
    ...
    if (! iszPLTWalletInitialized)
        zPLTWallet = new CzPLTWallet(strWalletFile);
    meta.isSeedCorrect = zPLTWallet->CheckSeed(dMint);
    if (! iszPLTWalletInitialized)
        delete zPLTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPLTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPLTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPLTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPLTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPLTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPLTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPLTWallet* zPLTWallet = new CzPLTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPLTWallet);
        }
        delete zPLTWallet;
        LogPrint("zero", "%s: added %d dzplt from DB\n", __func__, listDeterministicDB.size());
    }
