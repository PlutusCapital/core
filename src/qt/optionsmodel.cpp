...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PLT);
    ...
    if (settings.contains("nAnonymizePlutusAmount") || reset)
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in plutus.cpp during shotdown to also remove the window positions
